...
estimation_data <-
  ...
  select(seller_id,order_purchase_dt,order_id,product_id,review_score,
         product_name_lenght:avgrevscore,north,south) |> 
  arrange(seller_id,order_id)

```

Not everything is possible with Dplyr

```{r}
cluster_data <-
  merged_data_v2 |> 
  select(product_weight_g,product_length_cm,product_height_cm,product_width_cm)

cluster_data_sc <- scale(na.omit(cluster_data))
cluster_mod <- kmeans(cluster_data_sc, 3)
...
clusters_info <-
  ...
  select(order_id,clusters)

estimation_data2 <-
  estimation_data |> 
  left_join(clusters_info,"order_id")
```
