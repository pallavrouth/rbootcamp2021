...
```{r}
orders <- read_csv("~/Dropbox/Teaching/R bootcamp/Olist Ecom Proj/orders.csv")
glimpse(orders)
```
...
```{r}
order_items <- read_csv("~/Dropbox/Teaching/R bootcamp/Olist Ecom Proj/order_item.csv")
glimpse(order_items)
...
reviews <- read_csv("~/Dropbox/Teaching/R bootcamp/Olist Ecom Proj/reviews.csv")
glimpse(reviews)
...
products <- read_csv("~/Dropbox/Teaching/R bootcamp/Olist Ecom Proj/products.csv")
glimpse(products)

customers <- read_csv("~/Dropbox/Teaching/R bootcamp/Olist Ecom Proj/customers.csv")
glimpse(customers)

sellers <- read_excel("~/Dropbox/Teaching/R bootcamp/Olist Ecom Proj/sellers.xlsx",
...
Missing variables/renaming variables
...
Add these variables to estimation data
...
estimation_data <-
  ...
  mutate(delivary_time = order_delivered_customer_dt - order_purchase_dt,
         ...
         photos = case_when(product_photos_qty > 5 ~ "greater than 5",
                            TRUE ~ as.character(product_photos_qty))) |> 
  group_by(seller_id) |> 
    mutate(customersize = n_distinct(customer_id),
           productsize = n_distinct(product_id)) |> 
  ungroup() |> 
  group_by(seller_state) |> 
    mutate(sellersize = n_distinct(seller_id)) |> 
  ungroup() |> 
  group_by(product_category_name) |> 
    mutate(sellersize = n_distinct(seller_id)) |> 
  ungroup() |> 
  ...
  ungroup() |> 
  group_by(customer_state) |> 
    mutate(avgrevscore = mean(review_score,na.rm = T)) |> 
  ungroup() |> 
  mutate(row_no = 1:n(),
         indicator = 1) |> 
  pivot_wider(names_from = stateloc, values_from = indicator, values_fill = 0) |> 
  select(seller_id,order_purchase_dt,order_id,product_id,review_score,
         product_name_length:avgrevscore,north,south) |> 
  arrange(seller_id,order_id) |> 
  #type coersion
  mutate(delivary_time = as.numeric(delivary_time),
         approval_time = as.numeric(approval_time),
         photos = as.factor(photos),
         north = as.factor(north),
         south = as.factor(south)) 
```


Post data wrangling checks
...
plot_distribution <- function(x){
  data_plot <- estimation_data[,x]
  if (sapply(data_plot[,x],class) != "factor"){
...
1. Same background (PPT + Zoom)
2. Quick intro to layout
3. Using markdown (~30)
4. Viz at the end (using final variables)
5. Agenda = lecture objectives
6. Intro slide for yourself
7. By end of month